...
## 2-c from simple rotation excel file

devtools::load_all("/opt/R/src/socmor/")

f.xlsx.to.input.file(
    "./containers/2b_from_rotation_code/data-raw/SocMo_ag_rot_input_v3.xlsx"
)


l.my.rotation[[my.id]] <- do.call(
    expand_rotation_calendar,
    c(l.inputs.id,
      list(
          sowing.var = "SOWING_DATE",
          harvest.var = "HARVEST_DATE",
          start_year = 2020,
          n_years = n.year.max)
      ) ## 30 années de simulation ici !!!
) %>%
